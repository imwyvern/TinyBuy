...
app.ItemListView = Backbone.View.extend({
	...
	scrollToLoad: function() {
		...
		// 	headerHeight = $('#menu-container').find('.header').outerHeight);				//	顶部栏高度
		// 	marqueeHeight = $('#menu-container').find('.marquee').outerHeight();			//	跑马字幕高度
		// 	footerHeight = $('#menu-container').find('.toolbar').outerHeight();				//	底部栏高度
		// 	listInViewHeight = winHeight - headerHeight - footerHeight;		//	菜单列表可视区域高度
		var winHeight = $(window).height(),		// 当前窗口高度
			headerHeight = $('#menu-container').find('.header').height() + 10;				//	顶部栏高度
		...
		window.onscroll = function() {
			console.log("滚动了");
			// if (displayCount < that.viewCollection.length) {
			// 	var height = that.viewCollection[displayCount].$el.height();
			...
			// 	console.log("listInViewHeight:" + listInViewHeight);
			// 	console.log("anchor:" + anchor);

			// 	if ($(window).scrollTop() + listInViewHeight >= anchor + 51) {
			// 		if (loadNum == 1) {
			// 			that.viewCollection[displayCount++].imgLazyLoad();
			// 		} else if (loadNum == 2) {
			// 			that.viewCollection[displayCount++].imgLazyLoad();
			// 			if (that.viewCollection[displayCount])
			// 				that.viewCollection[displayCount++].imgLazyLoad();
			// 		}
