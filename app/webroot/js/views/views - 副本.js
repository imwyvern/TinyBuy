...
app.ItemView = Backbone.View.extend({
	...
	showDetail: function() {
		...
		var itemDetailView = new app.ItemDetailView({model: this.model, parent: that});
...
app.TuanView = Backbone.View.extend({
	...
	render: function() {
		this.$el.html(this.template(this.model.toJSON()));
		this.$el.attr('id', 'tuan-' + this.model.get('id'));
		this.imgLazyLoad();
		return this;
	},
			...
			success: function(data) {
				...
				if (rt_obj.code == 0) {
					...
					if (that.$el.find('.warning').length > 0) {
						...
						that.$el.find('.login-header').after("<p class=\"warning\">^_^用户名或密码错误，请重新登录</p>");
					}
				}
			},
			error: function(data) {
...
app.MenuView = Backbone.View.extend({
	...
	showVegetables: function() {
		...
		console.log($('#items-'+"item_"+$("#vegetable").attr("data")));
		if ($('#items-'+"item_"+$("#vegetable").attr("data")).length > 0) {
			$('#items-'+"item_"+$("#vegetable").attr("data")).show();
		} else {
			this.renderList("item_"+$("#vegetable").attr("data"));
		}		
	},
	...
	showMeat: function() {
		...
		$('.tuans').hide();
		$(window).scrollTop(0);
		if ($('#items-'+"item_"+$("#meat").attr("data")).length > 0) {
			$('#items-'+"item_"+$("#meat").attr("data")).show();
		} else {
			this.renderList("item_"+$("#meat").attr("data"));
		}		
	},
	...
	showSoup: function() {
		...
		$('.tuans').hide();
		$(window).scrollTop(0);
		if ($('#items-'+"item_"+$("#soup").attr("data")).length > 0) {
			$('#items-'+"item_"+$("#soup").attr("data")).show();
		} else {
			this.renderList("item_"+$("#soup").attr("data"));
		}		
	},
	...
	showSpeciality: function() {
		...
		$('.items').hide();
		$(window).scrollTop(0);
		if ($('#items-'+"item_"+$("#speciality").attr("data")).length > 0) {
			$('#items-'+"item_"+$("#speciality").attr("data")).show();
		} else {
			this.renderList("item_"+$("#speciality").attr("data"));
		}		
	}
});
...
/*
		else if (sc.getWechatId() && sc.getUuid()){
			that.$el.hide();
	...
	parseOrder: function() {
		if (this.code != 1 && this.code != 2) {
			...
			if (this.rt_data.cart) {
				...
				var deliver_date = this.deliver_date;
			...
			if (this.rt_data.tuan) {
				...
				for (var k in parsedObj) {
					...
					//order.set('order_status', (this.getDateStr(this.date, 1)) + status);
					order.set('order_status', (that.deliver_date) + status);
